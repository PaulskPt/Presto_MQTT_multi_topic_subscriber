...
def get_wx_timestamp(pl) -> str:
    global payload
    TAG = "wx_handle_ts(): "
    if pl is None:
        pl = payload # take the global variable

    if not my_debug:
        print(TAG+f"we\'re handling wx. type(pl) = {type(pl)}, payload = {pl}")
    if isinstance(pl, bytes):
        ...
        t0 = pl["metar"]
        t = t0["raw"]
        # print(TAG+f"t0 = {t0}, t = {t}")
    else:
        print(TAG+f"⚠️ expected payload type bytes or string, received: {type(pl)}")
        return # do nothing
    n = t.find("Z") # {"raw": "METAR LPPT 310100Z 33007KT CAVOK 27/11 Q1015"}
    if n >= 6:
        tm = t[n-6:n+1] # including 'Z'
        if my_debug:
            print(TAG+f"tm = {tm}")
        dd = tm[:2]
        hh = tm[2:6]
...
def mqtt_callback(topic: str, msg: bytes):
    global topic_rcvd, msg_rcvd, payload, last_update_time, topic_idx, redraw_done, sensor_obj, \
        toggle_obj, amb_obj, disp_obj, metar_obj, lightsDclrChanged
    TAG = "mqtt_callback(): "
    ...
    try:
        ...
        if isinstance(msg, bytes):
            if wx_test:
                payload = ujson.loads('{"metar": {"raw": "METAR LPPT 310100Z 33007KT CAVOK 27/11 Q1015"}}')
            else:
        ...
        if wx:
            ts = get_wx_timestamp(payload)
        else:
            if len(head) > 0:
                ts = head.get("t", datetime_empty)
            else:
                ts = datetime_empty
        if not my_debug:
            print("\n"+TAG+f"Received a mqtt message on topic: \"{topic_rcvd}\", timestamp: {ts}")
        if my_debug:
            print(f"msg: {msg}")
        if len(msg) > 0:
...
def save_to_obj(topIdx: int = 0, topic: str = "", payload: dict = {}) -> bool:
    ...
    if topIdx == 0:
        #sensor_obj = SensorTPAH()
        sensor_obj.head = head
        sensor_obj.topic = topic
        sensor_obj.topicIdx = topIdx
        sensor_obj.payload = payload.get("reads")
    elif topIdx == 1:
        #toggle_obj = LightsToggle()
        toggle_obj.topic = topic
        toggle_obj.topicIdx = topIdx
        toggle_obj.head = head
        toggle_obj.payload = payload.get("toggle")
    elif topIdx in (2,3):
        #amb_obj = AmbientColors()
        amb_obj.topic = topic
        amb_obj.topicIdx = topIdx
        amb_obj.head = head
        if topIdx == 2:
            amb_obj.payload = payload.get("colorInc")
        elif topIdx == 3:
            amb_obj.payload = payload.get("colorDecc")
    elif topIdx in (4,5):
        #disp_obj = DisplayColors()
        disp_obj.topic = topic
        disp_obj.topicIdx = topIdx
        disp_obj.head = head
        if topIdx == 4:
            disp_obj.payload = payload.get("dclrInc")
        elif topIdx == 5:
             disp_obj.payload = payload.get("dclrDec")
    elif topIdx == 6:
        #metar_obj = Metar()
        metar_obj.topic = topic
        metar_obj.topicIdx = topIdx
        metar_obj.head = head
        metar_obj.payload = payload.get("raw")
...
def split_msg():
    global topic_rcvd, payload, payloadLst, Publisher_ID, lightsColorIdx, lights_ON, lights_ON_old, lightsColorMax, \
...
def draw(mode:int = 1):
    ...
    if topic_idx == 6: # weather/metar
        time_draw = get_wx_timestamp(payload)
        if my_debug:
            print(TAG+f"wx time_draw = {time_draw}")
        hhS = time_draw[3:5]
        time_draw_hh = int(hhS) # 31 0100Z
        mmS = time_draw[5:7]
        time_draw_mm = int(mmS)
        if my_debug:
            print(TAG+f"time_draw[3:5] = {hhS}")
            print(TAG+f"time_draw[5:7] = {mmS}")
            print(TAG+f"time_draw_hh = {time_draw_hh}")
            print(TAG+f"time_draw_mm = {time_draw_mm}")
        time_draw = hhS + ":" + mmS + ":" + "00"
        if my_debug:
    ...
    if hh >= DISPLAY_HOUR_WAKEUP and hh < DISPLAY_HOUR_GOTOSLEEP:
        if CURRENT_COLOR is None:
            CURRENT_COLOR = ORANGE
            disp_obj.disp_color = CURRENT_COLOR # Save new display color setting
        else:
            CURRENT_COLOR = disp_obj.disp_color
        
        if (lightsDclrChanged):
            if not my_debug:
                print(TAG+f"lightsDclrChanged = {"True" if lightsDclrChanged else "False"}")
            #display.set_pen(CURRENT_COLOR)
            # Get the changed color value from the disp_obj
            CURR_COLOR = disp_obj.disp_color # Get the display color from disp_obj
            if CURR_COLOR in dispColorDict2.keys():
                pen_colorIdx = dispColorDict2[CURR_COLOR]
                if pen_colorIdx in dispColorNamesDict.keys():
                    pen_color_name = dispColorNamesDict[pen_colorIdx]
        ...
        # print(TAG+"we passed here: line 1800. Changing CURRENT_COLOR to NAVY")
        CURRENT_COLOR = NAVY
        disp_obj.disp_color = CURRENT_COLOR
        display.set_pen(CURRENT_COLOR)
    """
    ...
    if mode == 0:
        ...
        try:
            ...
            display.set_pen(CURRENT_COLOR)
            if topic_idx == 6:
                display.text(hdg, x, y, WIDTH, scale = my_scale)
            else:
                display.text(hdg + " " + time_draw, x, y, WIDTH, scale = my_scale)
            y += line_space
            if topic_idx == 6:
