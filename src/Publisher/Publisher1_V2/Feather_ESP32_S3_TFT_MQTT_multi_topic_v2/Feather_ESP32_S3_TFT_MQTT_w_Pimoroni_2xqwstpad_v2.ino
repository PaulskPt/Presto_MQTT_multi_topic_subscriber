/*
  ...
  Connected the two Qw/ST Pad game controllers to the second I2C port via GPIOs 10 and 11 while the other I2C devices remain on the default I2C port.
  Update 2025-08-26. The unixTime ("t") in the MQTT message sent by the Publisher(s) is now in UTC. It is up to the scripts of the Subscriber(s) to display
  the time in UTC or in local time. In this script is chosen to display in local time (hh:mm:ss+01:00 for Europe/Lisbon timezone).
*/
#include <Arduino.h>
#include <Unit_RTC.h>
    ...
    if (ckForButtonPress()) { // Check for button presses and handle them
      for (int i = 0; i < CURRENT_MAX_PADS; ++i) {
        if (padLogic[i].use_qwstpad) {
          ...
          //if (i == CURRENT_MAX_PADS-1 && btn_pressed)
          if (btn_pressed)
            break;
        }
      }
