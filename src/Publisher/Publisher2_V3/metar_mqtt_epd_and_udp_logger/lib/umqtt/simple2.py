...
class MQTTClient:
    ...
    def __init__(self, client_id, server, port=0, user=None, password=None, keepalive=0,
        ...
        if my_debug:
            print(f"MQTTClient.__init__(): server = {server}, port = {port}, client_id = {client_id}")
    ...
    def connect(self, clean_session=True):
        ...
        if self.ssl:
            import ssl # was: import ussl (doesn't exist in micropython on the Pimoroni pico lipo 2XL W)
            self.sock_raw.setblocking(True)
            self.sock = ssl.wrap_socket(self.sock_raw, **self.ssl_params) # was self.sock = ussl.wrap_socket ...
            self.sock_raw.setblocking(False)
        else:
