...
// Function added by @PaulskPt. Created by MS Copilot on Sat 2025-07-19.
int Unit_RTC::setUnixTime(unsigned long uxTime) {
    time_t unixTime = static_cast<time_t>(uxTime);  // Encapsulated conversion

    struct tm *utc = gmtime(&unixTime);
    if (utc == nullptr) return 0;

    rtc_date_type dateStruct(
        static_cast<uint16_t>(utc->tm_year + 1900),
    ...
    return setTime(&timeStruct) && setDate(&dateStruct);
}
